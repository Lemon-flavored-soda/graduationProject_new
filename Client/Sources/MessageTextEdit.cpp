...
//拖入事件
void MessageTextEdit::dragEnterEvent(QDragEnterEvent *e) {
    if(e->source() == this){
        e->ignore();
    }
    else{
...
void MessageTextEdit::dropEvent(QDropEvent *e) {
...
void MessageTextEdit::insertFromMimeData(const QMimeData *source) {
    ...
    if(urls.isEmpty())return;
    foreach(QString url, urls){
        if(isImage(url))insertImages(url);
        else insertTextFile(url);
    }
}

bool MessageTextEdit::isImage(QString url) {
    QString imageFormat = "bmp,jpg,png,tif,gif,pcx,tga,exif,fpx,svg,psd,dxf,ufo,eps,ai,raw,wmf,wrb";
    QStringList imageFormatList = imageFormat.split(",");
    QFileInfo fileInfo(url);
    QString suffix = fileInfo.suffix();
    if(imageFormatList.contains(suffix,Qt::CaseSensitive)){
        return true;
    }
    return false;
}
...
QPixmap MessageTextEdit::getFileIconPixmap(const QString &url) {
    ...
    int maxWidth =  FileSize.width() ? textSize.width() : FileSize.width();
